...
class OpenElevationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenElevationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenElevationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenElevationTestRunner.getenv("OPEN_ELEVATION_TEST_LIVE")
        override = OpenElevationTestRunner.getenv("OPEN_ELEVATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenElevationTestRunner.getenv(key)
        ...
        explain = OpenElevationTestRunner.getenv("OPEN_ELEVATION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPEN_ELEVATION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenElevationTestRunner._test_control is not None:
            return OpenElevationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenElevationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenElevationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenElevationTestRunner.load_test_control()
...
def load_env_local():
    OpenElevationTestRunner.load_env_local()


def env_override(m):
    return OpenElevationTestRunner.env_override(m)


def entity_data(v):
    return OpenElevationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenElevationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenElevationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenElevationTestRunner.load_test_control()


def live_delay_ms():
    return OpenElevationTestRunner.live_delay_ms()
