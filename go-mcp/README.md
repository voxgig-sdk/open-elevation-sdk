...
[MCP](https://modelcontextprotocol.io) server exposing the OpenElevation SDK as
two agent tools — `open-elevation_list` and `open-elevation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-elevation-mcp)
make build

# 2. Provide credentials via the environment
export OPEN_ELEVATION_APIKEY=sk_live_xxx
...
```jsonc
// open-elevation_list: first page of records
{ "entity": "lookup" }
{ "entity": "lookup", "query": { } }

// open-elevation_load: one record by id
{ "entity": "lookup", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-elevation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user open-elevation \
     -- "$PWD"/dist/*/open-elevation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `open-elevation_list` and `open-elevation_load` tools now appear
   in new sessions. Ask the agent to *"list lookup using open-elevation"*
   and it calls `open-elevation_list` with `{"entity":"lookup"}`.
...
```sh
export OPEN_ELEVATION_APIKEY=sk_live_xxx            # API key
export OPEN_ELEVATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `open-elevation_list` tool
...
### Call the `open-elevation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `open-elevation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `open-elevation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPEN_ELEVATION_APIKEY` | API key sent with every request. |
| `OPEN_ELEVATION_BASE` | Optional override of the API base URL. |
