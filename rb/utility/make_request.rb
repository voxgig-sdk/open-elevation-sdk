...
module OpenElevationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenElevationResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenElevationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenElevationResponse.new(fetched)
    else
