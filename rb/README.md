# OpenElevation Ruby SDK



The Ruby SDK for the OpenElevation API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Lookup` — with named operations (`list`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = OpenElevationSDK.test(testopts, sdkopts)
```
...
raise a `OpenElevationError` (a `StandardError` subclass), so wrap
...
```
rb/
├── OpenElevation_sdk.rb       -- Main SDK module
...
The main module (`OpenElevation_sdk`) exports the SDK class
...
# lookup.data_get now returns the lookup data from the last list
