...
The Ruby SDK for the OpenElevation API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.Lookup(nil).create({ "name" => "Example" }, nil)
...
result, err = client.OpenElevation(nil).load(
  { "id" => "test01" }, nil
)
