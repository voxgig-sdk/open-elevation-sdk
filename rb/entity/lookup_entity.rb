...
class LookupEntity
  ...
  def data_set(args)
    if args
      @_data = OpenElevationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OpenElevationHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = OpenElevationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
