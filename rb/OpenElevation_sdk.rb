...
require_relative 'OpenElevation_types'


class OpenElevationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenElevationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenElevationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenElevationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenElevationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenElevationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenElevationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenElevationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OpenElevationError.new(
        "#{op}_allow",
        "OpenElevationSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OpenElevationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenElevationError.new(
        "graphql_error", "OpenElevationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenElevationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
