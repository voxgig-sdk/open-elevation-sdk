...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OPEN_ELEVATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("OPEN_ELEVATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OPEN_ELEVATION_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.OpenElevationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.OpenElevationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.OpenElevationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.OpenElevationError{Msg: msg, Code: code}
}
