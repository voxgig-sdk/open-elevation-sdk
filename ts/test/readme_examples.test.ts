...
//     (\`new OpenElevationSDK(...)\` / \`OpenElevationSDK.test(...)\` become
//     \`OpenElevationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpenElevationSDK'
...
const TEST_SEED = {"entity":{"lookup":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenElevationSDK(...)\` and any
// \`OpenElevationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenElevationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenElevationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
