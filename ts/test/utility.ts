...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.OPENELEVATION_TEST_LIVE ||
    'TRUE' === process.env.OPENELEVATION_TEST_OVERRIDE
  ) {
  ...
  m.OPENELEVATION_TEST_EXPLAIN = process.env.OPENELEVATION_TEST_EXPLAIN || m.OPENELEVATION_TEST_EXPLAIN
