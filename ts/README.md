# OpenElevation TypeScript SDK



The TypeScript SDK for the OpenElevation API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Lookup()` — each with a small set of operations (`list`, `create`)
...
```ts
const lookups = await client.Lookup().list({ location: "example" })
...
```ts
const client = new OpenElevationSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `OpenElevationSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `OpenElevationSDK.test(testopts?, sdkopts?)` | `OpenElevationSDK` | Create a test-mode client. |
...
| `client` | `client(): OpenElevationSDK` | Return the parent SDK client. |
...
open-elevation/
├── src/
│   ├── OpenElevationSDK.ts        # Main SDK class
