...
boru-driven command-line client **and** interactive REPL for the OpenElevation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-elevation-cli)
make build

# 2. See usage (words, entities, env vars)
./open-elevation-cli --help

# 3. Provide credentials once, via the environment
export OPEN_ELEVATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./open-elevation-cli list lookup

# 5. Override the API base URL for a single call
OPEN_ELEVATION_BASE=https://api.example.com ./open-elevation-cli list lookup

# 6. No arguments -> interactive REPL
./open-elevation-cli
open-elevation> list lookup
open-elevation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-elevation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/open-elevation-cli list lookup
   ```
...
Run with no arguments to open a REPL (prompt `open-elevation>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `lookup`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/open-elevation-cli`. |
...
(`./open-elevation-cli <expr>`) and interactively (the REPL), and expressions compose the
