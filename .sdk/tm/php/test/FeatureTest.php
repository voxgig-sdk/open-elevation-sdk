...
// OpenElevation SDK feature test
//
...
class FtCtrl extends OpenElevationControl
{
...
class FtClient
{
    ...
    private OpenElevationUtility $utility;

    public function __construct(OpenElevationUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenElevationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenElevationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenElevationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenElevationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenElevationConfig::make_feature($name);
    ...
    private static function buildUrl(OpenElevationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenElevationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenElevationResponse($fetched);
        }
    ...
    private function fail(OpenElevationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenElevationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
