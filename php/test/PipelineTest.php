...
// OpenElevation SDK pipeline test
//
...
{
    private static function utility(): OpenElevationUtility
    {
        return new OpenElevationUtility();
    }
    ...
    private static function ctx(array $over = []): OpenElevationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenElevationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenElevationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenElevationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenElevationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenElevationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenElevationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenElevationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenElevationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenElevationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenElevationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenElevationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenElevationResponse([]),
            'result' => new OpenElevationResult([]),
        ]);
        [, $err] = OpenElevationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenElevationSpec([]),
            'response' => null,
            'result' => new OpenElevationResult([]),
        ]);
        [, $err] = OpenElevationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenElevationSpec(['step' => 's']),
            'response' => new OpenElevationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenElevationResult([]),
        ]);
        [, $err] = OpenElevationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenElevationSpec(['step' => 's']),
            'response' => new OpenElevationResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenElevationResult([]),
        ]);
        [, $err] = OpenElevationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenElevationSpec(['step' => 's']),
            'response' => new OpenElevationResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenElevationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenElevationMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenElevationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenElevationSpec([]),
            'response' => new OpenElevationResponse([]),
            'result' => new OpenElevationResult([]),
        ...
        [$got, $err] = OpenElevationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenElevationResult([])]);
        [, $err] = OpenElevationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenElevationSpec([]), 'result' => null]);
        [, $err] = OpenElevationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenElevationSpec(['step' => 's']),
            'result' => new OpenElevationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenElevationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenElevationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenElevationSpec(['step' => 's']),
            'result' => new OpenElevationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenElevationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenElevationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenElevationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenElevationSpec([]),
            'result' => new OpenElevationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenElevationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenElevationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenElevationError('boom', 'boom');
        $utility->fetcher = function (OpenElevationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenElevationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenElevationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenElevationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenElevationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenElevationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenElevationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenElevationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenElevationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenElevationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenElevationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenElevationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenElevationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenElevationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenElevationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenElevationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenElevationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenElevationResult(['ok' => false])]);
        $this->expectException(OpenElevationError::class);
        OpenElevationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenElevationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenElevationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenElevationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenElevationBaseFeature();
        $b = new OpenElevationBaseFeature();
        OpenElevationFeatureAdd::call($ctx, $a);
        OpenElevationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenElevationBaseFeature
    {
        $f = new OpenElevationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenElevationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenElevationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenElevationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenElevationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenElevationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenElevationResponse(['headers' => null]),
            'result' => new OpenElevationResult([]),
        ]);
        OpenElevationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenElevationResultBody::call($ctx);
