...
// OpenElevation SDK retry feature
...
class OpenElevationRetryFeature extends OpenElevationBaseFeature
{
    ...
    public function init(OpenElevationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenElevationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenElevationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenElevationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
